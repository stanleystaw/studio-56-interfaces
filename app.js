    ...
    <main id="main">
      <section class="hero" aria-labelledby="hero-title">
        <div class="container hero-grid">
          <div class="hero-content">
            ...
            <p class="hero-copy">56 inspirations d’interfaces réunies dans une seule galerie. Ouvre une référence pour voir son visuel complet, sans recadrage ni interprétation.</p>
            ...
            <div class="hero-note"><span></span> Les captures envoyées restent intactes dans les pages détail.</div>
          </div>
      ...
      <section class="stats-strip container" aria-label="À propos de la galerie">
        <div class="stats-panel">
          ...
          <div class="stat"><div><div class="stat-label">Fidélité à la source</div><div class="stat-value">100 % visuel</div></div><span class="stat-icon">${icon("image")}</span></div>
      ...
      <section class="catalog-section container" id="catalogue" aria-labelledby="catalogue-title">
        <div class="section-heading">
          ...
          <p>Chaque carte ouvre une page dédiée. Les visuels du tableau sont affichés tels qu’ils ont été transmis, en entier et sans recadrage.</p>
        </div>
      ...
      <section class="container" id="a-propos" style="padding:0 0 74px">
        <div class="info-card" style="display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:22px;padding:22px 25px">
          <div><div class="section-kicker" style="margin:0 0 8px">À propos du rendu</div><strong style="display:block;font-size:18px;letter-spacing:-.04em">La capture d’origine, pas une imitation approximative.</strong><p style="max-width:760px;margin:8px 0 0;color:var(--muted);font-size:11px;line-height:1.7">Pour respecter le visuel à l’identique, chaque fiche montre le fichier image correspondant en taille native quand l’écran le permet. Il s’agit d’une galerie de références visuelles : les éléments visibles dans les captures ne sont pas des sites indépendants recodés ni des formulaires connectés.</p></div>
...
function renderDetail(item) {
  ...
  document.title = `${item.title} — Studio 56`;
  app.innerHTML = `${header(true)}
    <main id="main" class="viewer-page container">
      <div class="breadcrumb"><a href="#/">Studio 56</a><span class="slash">/</span><a href="#/">Références</a><span class="slash">/</span><span>${item.id}</span></div>
      <div class="viewer-heading">
        <div class="viewer-title-block"><div class="viewer-label"><span class="eyebrow-dot"></span> Capture de référence · ${item.id}</div><h1>${escapeHTML(item.title)}</h1><p class="viewer-subtitle">${escapeHTML(item.description)}</p></div>
        <div class="viewer-pagination-top"><span class="pager-count"><strong>${item.id}</strong> / 56</span><a class="icon-button" href="#/interface/${previous.id}" aria-label="Référence précédente">${icon("chevronLeft")}</a><a class="icon-button" href="#/interface/${next.id}" aria-label="Référence suivante">${icon("chevronRight")}</a></div>
      </div>
      <div class="viewer-layout">
        <div class="capture-frame">
          <div class="capture-chrome"><span class="chrome-lights"><i></i><i></i><i></i></span><span>Visuel source · non recadré</span><span>REF ${item.id}</span></div>
          <div class="capture-surface"><img class="capture-image" src="${item.asset}" alt="Capture complète : ${escapeHTML(item.title)}" decoding="async"></div>
        </div>
        <aside class="viewer-aside" aria-label="Informations sur le visuel">
          <section class="info-card">
            <div class="info-kicker">Dans la collection</div><span class="category-pill"><i></i>${escapeHTML(item.category)}</span>
            <p class="info-description">${escapeHTML(item.description)}</p>
            <div class="detail-divider"></div>
            <div class="source-status">${icon("check")}<span>Image fournie affichée <strong>sans recadrage</strong>, à sa largeur native si l’écran le permet.</span></div>
            <div class="detail-divider"></div>
            <div class="file-meta"><span>Référence</span><strong>${item.id} / 56</strong><span>Fichier</span><strong>ref-${item.id}.jpg</strong><span>Source</span><strong>Visuel fourni</strong></div>
            <div class="aside-actions">
              <a class="button button-primary" href="${item.asset}" download="ref-${item.id}.jpg">${icon("download")} Télécharger la capture</a>
              <button type="button" class="button button-quiet" data-copy>${icon("copy")} Copier le lien de cette page</button>
            </div>
          </section>
          <div class="viewer-shortcuts"><kbd>←</kbd><kbd>→</kbd> pour changer de référence · <kbd>Esc</kbd> pour revenir</div>
        </aside>
      </div>
      <nav class="viewer-bottom-nav" aria-label="Navigation entre les références">
        <a class="text-link" href="#/interface/${previous.id}">${icon("arrowLeft")}<span>${escapeHTML(previous.title)}</span></a>
        <span class="center-note">Visuel ${item.id} sur ${references.length}</span>
        <a class="text-link" href="#/interface/${next.id}"><span>${escapeHTML(next.title)}</span>${icon("arrowRight")}</a>
      </nav>
    </main>
    ${footer()}
    <div class="toast" id="toast" role="status">${icon("check")}<span>Lien copié dans le presse-papiers.</span></div>`;

  app.querySelector("[data-copy]").addEventListener("click", copyCurrentLink);
  const image = app.querySelector(".capture-image");
  image.addEventListener("error", () => {
    image.alt = "La capture ne peut pas être chargée.";
    image.style.minHeight = "300px";
    image.style.background = "#f1f1f1";
  }, { once: true });
}
